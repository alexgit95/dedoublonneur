...
4. **Traitement** (onglet Export de `/`) — le bouton de previsualisation calcule d'abord les photos conservees/supprimees, les videos et l'economie estimee sans copier ; `POST /api/jobs/{id}/process` copie ensuite (octet a octet, metadonnees EXIF/GPS/dates preservees) toutes les photos conservees et toutes les videos vers un nouveau dossier de sortie (rejet si le nom existe deja). La progression est consultable via `GET /api/jobs/{id}/processing-progress` et le recapitulatif final via `GET /api/jobs/{id}/result` ; le dossier source n'est jamais modifie.
